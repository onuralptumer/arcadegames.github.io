...
export class Kalkan {
    ...
    init(o) {
        ...
        for (let i = 0; i < 14; i++)
            this.blasts.push({ x: 0, y: 0, t: 0, live: false });
        this.reset();
    }
    reset() {
        this.wave = 1;
        ...
        for (const m of this.missiles) {
            ...
            if (m.y >= this.groundY) {
                ...
                for (const base of this.bases) {
                    if (base.alive && Math.abs(base.x - m.x) < this.baseW)
                        base.alive = false;
                }
                if (!this.bases.some((b) => b.alive))
                    return this.die();
            }
        }
    ...
    render() {
        ...
        for (const b of this.bases) {
            if (b.alive)
                ctx.fillRect(b.x - this.baseW / 2, this.groundY - this.baseH, this.baseW, this.baseH);
        }
        for (const b of this.blasts) {
            if (!b.live)
                continue;
