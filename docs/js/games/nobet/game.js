export class Nobet {
    ...
    init(o) {
        ...
        for (let i = 0; i < 24; i++)
            this.shots.push({ x: 0, y: 0, vy: 0, live: false });
        for (let i = 0; i < 16; i++)
            this.bombs.push({ x: 0, y: 0, vy: 0, live: false });
        this.reset();
    }
    reset() {
        this.wave = 1;
    ...
    update(dt) {
        ...
        for (const e of this.enemies) {
            if (e.alive && e.y + this.eh >= this.py)
                return this.die();
        }
        if (this.alive() === 0) {
            this.wave++;
    ...
    render(alpha) {
        ...
        for (const e of this.enemies) {
            if (e.alive)
                ctx.fillRect(e.x, e.y, this.ew, this.eh);
        }
        for (const b of this.bombs) {
            if (b.live)
                ctx.fillRect(b.x - this.W * 0.006, b.y, this.W * 0.012, this.H * 0.028);
        }
        ...
        for (const s of this.shots) {
            if (s.live)
                ctx.fillRect(s.x - this.W * 0.006, s.y, this.W * 0.012, this.H * 0.03);
        }
    }
    destroy() {
        this.enemies = [];
