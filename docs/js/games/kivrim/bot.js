...
export class KivrimBot {
    ...
    poll(s) {
        ...
        if (Math.abs(dx) >= Math.abs(dy)) {
            if (dx !== 0)
                want.push(dx > 0 ? 'right' : 'left');
            if (dy !== 0)
                want.push(dy > 0 ? 'down' : 'up');
        }
        else {
            if (dy !== 0)
                want.push(dy > 0 ? 'down' : 'up');
        ...
        if (this.rng() > this.skill) {
            const opts = Object.keys(free).filter((d) => free[d]);
            if (opts.length)
                pick = opts[Math.floor(this.rng() * opts.length)];
        }
        if (!pick)
            pick = want.find((d) => free[d]) ?? null;
        const cur = s.dirX === 1 ? 'right' : s.dirX === -1 ? 'left' : s.dirY === 1 ? 'down' : 'up';
