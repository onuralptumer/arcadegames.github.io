...
export class Katla {
    ...
    sync() {
        ...
        for (let y = 0; y < N; y++) {
            for (let x = 0; x < N; x++) {
                ...
                if (v)
                    this.tiles.push({ v, fx: x, fy: y, x, y, born: false });
            }
        }
    }
    canMove() {
        for (let i = 0; i < N * N; i++) {
            ...
            const x = i % N;
            const y = (i / N) | 0;
            if (x + 1 < N && this.grid[i] === this.grid[i + 1])
                return true;
            if (y + 1 < N && this.grid[i] === this.grid[i + N])
                return true;
        }
        return false;
    }
    ...
    snapshot() {
        const s = { score: this.score, over: this.over ? 1 : 0, busy: this.anim > 0 ? 1 : 0 };
        for (let i = 0; i < N * N; i++)
            s[`c${i}`] = this.grid[i];
        return s;
    }
    update(dt) {
        ...
        if (this.over) {
            this.overFor += dt;
            if (this.o.mode === 'demo' && this.overFor > 1.2)
                this.reset();
            return;
        }
