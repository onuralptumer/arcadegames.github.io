...
export class Deflekt implements Game {
  private o!: GameInit;
  private W = 0;
  private H = 0;
  ...
  update(dt: number) {
    ...
    if (!this.launched) {
      ...
      if (it.action || this.o.mode === 'demo') this.launch();
      return;
    }
  ...
  private hitBricks() {
    for (let i = 0; i < this.bricks.length; i++) {
      ...
      if (this.bricks.length === 0) {
        ...
        if (this.o.mode !== 'demo') this.o.events.levelUp(next + 1);
        this.loadLevel(next);
      }
      return;
    }
  ...
  render(alpha: number) {
    ...
    for (const b of this.bricks) if (b.hp <= 1) ctx.fillRect(b.x, b.y, b.w, b.h);
