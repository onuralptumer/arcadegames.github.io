...
interface Lane {
  ...
  dir: 1 | -1;
  speed: number;
  /** Engellerin sol kenarları, hücre biriminde. */
  items: number[];
  len: number;
}

export class Gecit implements Game {
  private o!: GameInit;
  private cols = 0;
  private rows = 0;
  private cell = 0;
  ...
  private buildLanes() {
    ...
    for (let y = 1; y < this.rows - 1; y++) {
      ...
      for (let x = 0; x < this.cols; x += gap) items.push(x + this.o.rng());
  ...
  private safeAt(x: number, y: number): number {
  ...
  private safeFor(l: Lane, col: number): number {
    if (this.occupied(l, col)) return 0;
  ...
  private occupied(l: Lane, col: number): boolean {
    for (const x of l.items) {
      if (col + 0.75 > x && col + 0.25 < x + l.len) return true;
    }
    return false;
  }
  ...
  update(dt: number) {
    if (this.stun > 0) {
      this.stun -= dt;
      if (this.stun <= 0) this.reset(false);
      return;
    }

    const it = this.o.input.poll(this.snapshot());
    if (it.dir === 'up' && this.py > 0) this.py--;
    else if (it.dir === 'down' && this.py < this.rows - 1) this.py++;
    else if (it.dir === 'left' && this.px > 0) this.px--;
    else if (it.dir === 'right' && this.px < this.cols - 1) this.px++;

    for (const l of this.lanes) {
      for (let i = 0; i < l.items.length; i++) {
        l.items[i] += l.dir * l.speed * dt;
        if (l.dir > 0 && l.items[i] > this.cols) l.items[i] = -l.len;
        else if (l.dir < 0 && l.items[i] < -l.len) l.items[i] = this.cols;
      }
      if (l.y === this.py && this.occupied(l, this.px)) return this.die();
    }
  ...
  render() {
    ...
    if (this.stun > 0 && Math.floor(this.stun * 12) % 2 === 0) return;
