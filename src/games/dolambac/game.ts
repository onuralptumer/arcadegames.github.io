...
interface Chaser {
  ...
  y: number;
  dir: Dir;
  /** 0 doğrudan takip, 1 önünü kesme, 2 rastgele. */
  kind: number;
  scared: number;
}

export class Dolambac implements Game {
  private o!: GameInit;
  private cols = 0;
  private rows = 0;
  private cell = 0;
  ...
  private pellets!: Uint8Array; // 0 yok, 1 yem, 2 güç yemi
  private left = 0;
  ...
  private nearestPellet(): [number, number] {
    const seen = new Uint8Array(this.cols * this.rows);
    const q: number[] = [this.py * this.cols + this.px];
    seen[q[0]] = 1;
    let head = 0;
    while (head < q.length) {
      const i = q[head++];
  ...
  private stepPlayer() {
    ...
    if (this.pellets[i]) {
      ...
      if (this.pellets[i] === 2) for (const c of this.chasers) c.scared = 6;
      this.pellets[i] = 0;
      this.left--;
      if (this.o.mode !== 'demo') this.o.events.score(this.score);
      if (this.left === 0) {
  ...
  private stepChasers() {
    for (const c of this.chasers) {
      ...
      for (const d of ['up', 'down', 'left', 'right'] as Dir[]) {
        ...
        if (this.opposite(d) === c.dir) continue;
        if (!this.wall(c.x + DIRS[d][0], c.y + DIRS[d][1])) opts.push(d);
      }
      if (!opts.length) {
